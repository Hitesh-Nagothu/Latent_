...
##
#  Create a new directory new_data, inside it:
...
def write_reply_count(reply_tracker, files):
    ...
    for file in files:
        print(file)
        new_data = []
        with open ('data_tweets/' + file, "r") as f:
            data = json.load(f)
        with open('processed_data_tweets/' + file, "w") as f:
            for d in data:
                t = {}
...
files = [f for f in listdir('data_tweets/') if 'json' in f]

reply_tracker = create_verified_data(files)

reply_files = [f for f in listdir('data_replies/') if 'json' in f]
